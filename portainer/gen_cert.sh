...
openssl genrsa -des3 -out serverkey.pem -passout pass:$PASS 2048
openssl req -new -key serverkey.pem -out server.csr -passin pass:$PASS -subj '/CN=*/'

# sign the server key with our CA
openssl x509 -req -days $DAYS -passin pass:$PASS -in server.csr -CA ca.pem -CAkey ca-key.pem -out server.pem

# create a client key and certificate signing request (CSR)
openssl genrsa -des3 -out clientkey.pem -passout pass:$PASS 2048
openssl req -subj '/CN=client' -new -key clientkey.pem -out client.csr -passin pass:$PASS

# create an extensions config file and sign
echo extendedKeyUsage = clientAuth > extfile.cnf
openssl x509 -req -days $DAYS -passin pass:$PASS -in client.csr -CA ca.pem -CAkey ca-key.pem -out client.pem -extfile extfile.cnf

# remove the passphrase from the client and server key
openssl rsa -in serverkey.pem -out serverkey.pem -passin pass:$PASS
openssl rsa -in clientkey.pem -out clientkey.pem -passin pass:$PASS
